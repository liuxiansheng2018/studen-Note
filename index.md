...
###事件循环（event-loop）
        是说事件循环之前我们要先了解我们的javascript是一个单线程的,如果javascript不是单线程的话,一个线程给我们dom节点添加内容,一个线程在给我们的节点删除内容,这样就导致了我们乱套了,所以javascript是单线程的。但是我们javascript有同步和异步的概念,解决了js堵塞的问题
        ###同步:
            一个函数在返回的时候，调用者就能拿到预期的结果
        ###异步:
            在一个函数返回的时候,调用者还拿不到预期的结果，需要在将来通过一定的手段才能拿到
        主线程循环:
            主线程会不停的从执行栈中读取事件,会完成执行栈的所有的同步代码，当遇到一个异步任务的时候，并不会一直去等待他，而是把异步任务中的事件放到与执行栈不同的任务队列当中，当主线程将执行栈中的代码执行完毕之后，回去查看任务队列中是否有任务,如果有任务,那么主线程会依次执行任务队列中的回调函数,当执行完以后在执行下一个执行栈
        js异步的循环机制:
            1. 所有的任务都在主线程上执行，形成一个执行栈
            2. 主线程之外还有一个任务队列, 只要异步任务有了返回结果啊，就在任务队列中放置一个事件
            3. 当一旦执行栈的同步任务执行完毕之后，系统就会读取任务队列中的事件,那些对应的异步任务结束等待,进入执行栈开始执行
            4. 主线程重复上面的3步骤
        异步任务分为宏任务和微任务
        宏任务:
            script（整体代码）, setTimeout, setInterval, ui , I/o
        微任务:
            promise.then(), MutaionObserver,
        事件循环:
            1. 执行栈选择最先进入队列的宏任务（通常是script），如果有则执行
            2. 检查是否有微任务，如果存在则不停的执行，直到清空微任务
            3. 更新render(每一次的事件循环，浏览器都有可能更新渲染)
            4. 重复以上步骤
###cookie中的http-only
    如果cookie中设置了HttpOnly,那么通过js脚本是无法读取到cookie信息的,有效防止xss攻击，窃取cookie
    HttpOnlay设置的样例:
        response.setHeader("Set-Cookie" : "cookiename=httponlyTest; Path=/; Domain=domainvalue; Max-Age=seconds; HttpOnly");
    但是通过使用以下的方式可以拿到
     Cookie cokies[] = request.getCookie();
###this绑定的方式
    1.默认绑定
          当一个函数的函数没有明确的调用对象的时候，也就单纯的作为单独的函数使用，将对函数的this使用默认绑定: 绑定到window对象为止  
    2.显示绑定
          使用call,apply或者bind的方式指定上下文,函数中的this会绑定到传入的对象中
    3.隐式绑定
          函数调用的时候you有上下文对象,那么这个this会绑定到这个上下文对象,但是隐式绑定会丢失
          隐式绑定this丢失两个情况:
            1. 赋值丢失（绑定上下文的对象的函数被赋值给一个新的函数） var obj1 = obj.foo;  obj1();
            2. 传参丢失 setTimeout( obj.foo, 100 )
    4.new绑定  
        var obj = {};
        obj.__proto__ = constructorFn.prototype;
        constructorFn.call(obj)
          1. 创建一个新对象
          2. 将构造函数的作用域赋值给新对象(将实例本身的__proto__属性指向一个构造函数的原型, obj新增了构造函数prototyp对象上的挂载的属性和方法,并打构造函数的this指向替换成obj)
          3. 执行构造函数中的代码
          4. 返回新对象
###calc()
    用来动态的计算长度的值
...
###tcp三次握手
    客户端向服务器发送一个报文
    服务端收到报文 确认自己可以收到客户端发来的报文 发送一个报文给客户端
    客户端收到服务端发来的报文 可以确认自己可以收到服务器发来的报文 然后发送一个报文给服务端
    服务端收到报文 确认客户端收到了上次自己发送的报文 确定双方可连接 连接开始
###tcp4次挥手
    客户端告诉服务端要关闭连接了
    服务端收到客户端关闭请求 服务端回复我收到了你的关闭 但我需要等待所有数据传输结束
    服务器数据传输结束 向客户端发送结束报文 我要关闭连接
    客户端 收到报文 发送报文给服务端 让服务端确认关闭
